...
-i or --ifile    输入配置文件路径，默认是当前目录:YHGZ-Template-Parameters.xlsx
-t or --template 输入模板文件路径，默认为当前目录:YHGZ-Template-V9.docx
'''
def pwd():
    pwd = os.path.split(os.path.realpath(__file__))[0]
    return pwd;
def get(argv):
   inputfile = pwd() + u'\\YHGZ-Template-Parameters.xlsx'
   inputfile_T = pwd() + u"\\YHGZ-Template-V9.docx"
   outputpath = pwd()
   try:
...
def conf(pathxlsx):
    ...
    lists = wb.sheets[0].range('B1:B9').value
    wb.close()
    app_xlsx.quit()
    for x in lists:
        print u'成功获取: "%s"...' % x
...
def replace(pathword,outputpath):
    ...
    NewData3 = lists[6]
    OldIp1 = u'XXX.XXX.XXX'
    NewIp1 = lists[7]
    OldIp2 = u'XX.XX.XX.XX'
    NewIp2 = lists[8]
    ...
    app.Selection.Find.Execute(OldIp1, False, False, False, False, False, True, 1, False, NewIp1, 2)
    app.Selection.Find.Execute(OldIp2, False, False, False, False, False, True, 1, False, NewIp2, 2)
    print u"正在保存模板中..."
